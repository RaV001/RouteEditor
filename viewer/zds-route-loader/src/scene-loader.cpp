...
{   
    std::string track_name = routeDir + separator() +  "route";
...
ReadResult SceneLoader::loadObjectRef(std::istream &stream)
{
    ...
    while (!stream.eof())
    {
        ...
        object.model_path = toNativeSeparators(object.model_path);
        object.texture_path = toNativeSeparators(object.texture_path);
